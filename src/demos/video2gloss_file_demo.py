"""
Video2Gloss File Demo
Select video file -> Detect glosses with live playback
"""
...
class VideoFileProcessor(QObject):
    """Processes video file and detects glosses with live playback"""
...
class Video2GlossFileWidget(QWidget):
    """Main widget for file selection and gloss detection"""
    ...
    def setup_ui(self):
        ...
        # Title
        title = QLabel("Video to Gloss - File Demo")
        title.setStyleSheet("font-size: 18px; font-weight: bold; padding: 10px;")
        layout.addWidget(title)

        # Status
        self.status_label = QLabel("✓ Ready - Select a video file")
        ...
        # Video display
        self.video_display = VideoDisplayLabel("Video playback will appear here")
    ...
    def select_video(self):
        ...
        if file_path:
            ...
            self.sequence_display.setText("Click 'Process Video' to begin")
            self.prev_btn.setEnabled(False)
            self.next_btn.setEnabled(False)
    ...
    def process_video(self):
        ...
        self.current_gloss_label.setText("---")
        self.current_confidence_label.setText("")
        self.sequence_display.setText("Processing... detections will appear here")
    ...
    def on_gloss_detected(self, gloss, confidence):
        ...
        self.update_status_signal.emit(
            f"▶ Processing... detected {len(self.detected_sequences)} signs"
        )
    ...
    def on_processing_complete(self):
        ...
        if self.detected_sequences:
            self.update_status_signal.emit(
                f"✓ Processing complete - {len(self.detected_sequences)} signs detected"
            )
            self.prev_btn.setEnabled(len(self.detected_sequences) > 5)
        else:
            self.update_status_signal.emit("✓ Processing complete - No signs detected")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Video to Gloss - File Demo")
        self.setGeometry(100, 100, 900, 800)
        self.setStyleSheet(get_dark_stylesheet())

        widget = Video2GlossFileWidget()
