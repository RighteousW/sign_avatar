...
from gesture_recognizer_model_training import SignLanguageModel
...
class MediapipeProcessor:
    """Simplified MediaPipe processor for hand and pose landmark detection"""

    def __init__(self, process_hands=True, process_pose=True):
        self.process_hands = process_hands
        ...
        if process_hands:
            hand_options = vision.HandLandmarkerOptions(
                base_options=python.BaseOptions(
                    model_asset_path=str(MEDIAPIPE_HAND_LANDMARKER_PATH)
                ),
        ...
        if process_pose:
            pose_options = vision.PoseLandmarkerOptions(
                base_options=python.BaseOptions(
                    model_asset_path=str(MEDIAPIPE_POSE_LANDMARKER_PATH)
                ),
                running_mode=vision.RunningMode.VIDEO,
                num_poses=1,
    ...
    def process(self, frame):
        """
        Process a frame and return hand and pose landmarks.
        Returns None for results not processed.
        """
        ...
        # Detect hands
        hand_result = None
        if self.process_hands and self.hand_landmarker:
        ...
        # Detect pose
        pose_result = None
        if self.process_pose and self.pose_landmarker:
...
def main():
    ...
    print("Initializing MediaPipe and model...")
    mediapipe_processor = MediapipeProcessor(process_hands=True, process_pose=True)
